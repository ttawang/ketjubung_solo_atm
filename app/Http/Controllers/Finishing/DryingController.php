...
{
    private static $model = 'Drying';
    private static $modelDetail = 'DryingDetail';

    public function index()
    {
        $data['breadcumbs'] = [['nama' => 'Finishing', 'link' => 'javascript:void(0)', 'active' => ''], ['nama' => 'Drying', 'link' => 'javascript:void(0)', 'active' => 'active']];
        $data['menuAssets'] = menuAssets('finishing', 'drying', $data['breadcumbs'], true, true, true, true);
        return view('contents.production.finishing.drying.index', $data);
    }
    public function view()
    {
        $data['grade'] = $this->getGrade();
        return view('contents.production.finishing.drying.parent', $data);
    }
    public function table(Request $request)
    {
        $tanggal = $request->tanggal ?? null;
        $temp = DryingDetail::when($tanggal, function ($q) use ($tanggal) {
        ...
        return DataTables::of($temp)
            ...
            ->addColumn('action', function ($i) {
                ...
                /*$cem = ChemicalFinishing::where([['id_detail', $i->id], ['code', 'CD']])->count();
        ...
        try {
            ...
            if ($rule['success'] == false) {
                ...
                if (!$id) {
                    ...
                    DryingDetail::create($data);
                    logHistory(self::$modelDetail, 'create');
                } else {
                    $inspect = DryingDetail::find($id);
                    ...
                    $data = unsetMultiKeys(['id_grade_awal'], $data);
                    DryingDetail::find($id)->update($data);
    ...
    public function hapus($id)
    {
        ...
        try {
            ...
            $logIdOut = DryingDetail::find($id)->id_log_stok_penerimaan_keluar;
            $logIdIn = DryingDetail::find($id)->id_log_stok_penerimaan_masuk;
            DryingDetail::find($id)->delete();
    ...
    {
        $data = getDataJasaDalam($request, 'drying');
        return $data;
    }
    function getBarang(Request $request)
    {
        ...
        $motifKhusus = implode(',', motifKhususAsset('finishing'));
        $condition = [
            'id_satuan_1' => 4,
            'id_gudang' => $gudang ?? 0,
            'code_custom' => "CASE WHEN code = 'IP1' THEN id_motif IN ($motifKhusus) ELSE code IN ('JCS', 'BGLG') END"
        ];
    ...
    function getGudang(Request $request)
    {
        ...
        $motifKhusus = implode(',', motifKhususAsset('finishing'));
        $atribut = [
            'code_custom' => "CASE WHEN data.code = 'IP1' THEN id_motif IN ($motifKhusus) ELSE data.code = 'JCS' END",
            'search' => $term
        ];
    ...
    {
        return getStokJasaLuar($request, 'drying');
    }
}
