<form action="{{ isset($id) ? route('production.dyeing_gresik.update', $id) : route('production.dyeing_gresik.store') }}"
    ...
    <input type="hidden" name="input[id_dyeing_gresik]" value="{{ $idParent }}">
    ...
    <div class="form-group">
        ...
        <select name="input[id_gudang]" id="select_gudang_2" onchange="changeGudang($(this))"
            data-placeholder="-- Pilih Gudang --" data-model="LogStokPenerimaan" data-id-gudang-selected="2"
            data-route="{{ route('helper.getGudang') }}" class="form-control select2" required></select>
    </div>
    <div class="form-group">
        <label>Jenis Benang</label>
        @if ($code == 'BBDG')
            <select id="select_barang" onchange="changeBarang($(this));" data-filter="1" data-filter-code="BBDG"
        ...
        @else
            <select id="select_barang" name="input[id_parent_detail]" data-id-dyeing-jasa-luar="{{ $idParent }}"
                onchange="changeBarangTerima($(this));" data-route="{{ route('helper.getDyeingGresik') }}"
    ...
    @if ($code == 'BDG')
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control number-only" name="input[volume_2]" required>
            <div id="wrapperSuggestionStok1"></div>
        </div>
    @else
        <div class="form-group">
