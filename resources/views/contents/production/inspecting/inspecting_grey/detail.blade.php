...
<form id="form" action="" method="POST" enctype="multipart/form-data">
    ...
    <input type="hidden" name="id_tenun_detail" id="id_tenun_detail" value="{{ $id_tenun_detail }}">
...
@if (Auth::user()->roles_name !== 'validator')
    @if (!$data->validated_at)
        <button type="button" class="btn btn-primary" onclick="simpanKualitas($(this))"
            data-id_inspecting_grey="0">Simpan</button>
    @endif
@endif

<script type="text/javascript">
    var id_tenun_detail = '{{ $id_tenun_detail }}';
    $(function() {
        $('.select2').select2({
    ...
    function table() {
        table = $('#table-detail').DataTable({
            ...
            autoWidth: false,
            order: [],
            ajax: `inspect_grey/table/${mode}/${id_tenun_detail}`,
