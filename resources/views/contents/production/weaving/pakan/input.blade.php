        ...
        <div class="col-md-12">
            @if (Auth::user()->roles_name !== 'validator')
                @if (!$data->validated_at)
                    ...
                    <button type="button" class="btn btn-primary btn-sm waves-effect waves-classic float-left mr-2"
                        onclick="tambahPengirimanPakan($(this))" data-id="{{ $data->id }}"
                        data-tanggal="{{ $data->tanggal }}">
                        <i class="icon md-refresh-sync mr-2"></i> Tambah Benang Warna Pakan dari Pengiriman</button>
                @endif
            @endif
        </div>
    </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="padding-bottom: 20px;">
                <form class="form-horizontal" id="form" action="" method="POST"
                    ...
                    <input type="hidden" name="code" id="code" value="BBWP">
...
<script type="text/javascript">
    ...
    function getStok(barang, warna, gudang, id = null) {
        if (warna != 0 && barang != 0 && gudang != 0) {
            $.get(`pakan/get-stok-barang/${barang}/${warna}/${gudang}/${tipe}`, function(data) {
                if (!id) {
    ...
    function tambahPengirimanPakan(element) {
        Swal.fire({
            ...
            if (result.value === true) {
                let idPakan = element.data('id');
                let tanggal = element.data('tanggal');
                $.ajax({
                    url: "{{ route('helper.inputPakanKirim') }}",
                    type: "POST",
                    data: {
                        id: idPakan,
                        tanggal: tanggal
