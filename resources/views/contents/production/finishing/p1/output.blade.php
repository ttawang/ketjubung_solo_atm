        ...
        <div class="col-md-12">
            @if (Auth::user()->roles_name !== 'validator')
                @if (!$data->validated_at)
                    ...
                    <button type="button" class="btn btn-warning btn-sm waves-effect waves-classic float-left mr-2" onclick="terimaSemuaBarangJasaLuar($(this))" data-id="{{ $data->id }}"
                        data-proses="p1">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="padding-bottom: 20px;">
                <form class="form-horizontal" id="form" action="" method="POST" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="id_p1" id="id_p1">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="simpan($(this))" data-proses="p1">Simpan</button>
            </div>
        </div>
    </div>
</div>
                ...
                processResults: function(data) {
                    return {
                        results: data.data.map(function(data) {
                            ...
                            return {
                                id: data.id,
                                text: `${data.id_mesin ? data.nama_mesin+' | ' : ''}${data.id_beam ? data.nomor_kikw + ' | ' : ''}${data.id_songket? data.nomor_kiks + ' | ' : ''}${data.nama_barang} | ${data.nama_warna} | ${data.nama_motif} | ${data.nama_grade} | ${data.tanggal}${(idInspectRetur != '') ? ' | RETUR' : ''}${data.tanggal_potong ? ' | '+data.tanggal_potong_text:''}`,
                                data: {
                ...
                },
{
                ...
                },
                {
                    ...
                    name: 'status',
                    render: (data) => {
                        let jumlahInspekting = data.jumlah_inspecting_p1 || 0;
        ...
        $.ajax({
            ...
            success: function(respon) {
                ...
                $(`#barang`).select2("trigger", "select", {
                    data: {
                        id: respon.id_parent,
                        text: `${respon.id_mesin ? respon.nama_mesin+' | ' : ''}${respon.id_beam ? respon.nomor_kikw + ' | ' : ''}${respon.id_songket ? respon.nomor_kiks + ' | ' : ''}${respon.nama_barang} | ${respon.nama_warna} | ${respon.nama_motif} | ${respon.nama_grade_awal}${respon.tanggal_potong ? ' | '+respon.tanggal_potong_text:''}`,
                        data: {
                            ...
                            id_parent: respon.id_parent,
                            tanggal_potong: respon.tanggal_potong,
                        }
                    }
                });
                editbarang = `${respon.id_mesin},${respon.id_beam},${respon.id_songket},${respon.id_barang},${respon.id_warna},${respon.id_motif},${respon.id_grade_awal},${respon.tanggal_potong}`;
                $('#grade').val(respon.id_grade).trigger('change');
        ...
        $.ajax({
            ...
            success: function(respon) {
                ...
                if (id == "") {
                    ...
                    var tempbarang = `${data.id_mesin},${data.id_beam},${data.id_songket},${data.id_barang},${data.id_warna},${data.id_motif},${data.id_grade},${data.tanggal_potong}`;
                    if (editgudang != data.id_gudang || editbarang != tempbarang) {
                        $('#stok_1').text(respon.stok_1);
                        $('#stok_2').text(respon.stok_2);
                    } else {
                        $('#stok_1').text(parseFloat(parseFloat(respon.stok_1) + parseFloat($('#volume_1').val())));
                        $('#stok_2').text(parseFloat(parseFloat(respon.stok_2) + parseFloat($('#volume_2').val())));
                    }
                }
            },
            error: function(error) {
