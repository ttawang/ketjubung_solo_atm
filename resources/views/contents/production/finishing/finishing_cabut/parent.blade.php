        ...
        <table class="table table-bordered table-hover table-striped" cellspacing="0" id="table-parent">
            <thead>
                <tr>
                    ...
                    <th>No. SPK</th>
                    <th>Vendor</th>
                    <th width="100px">Total Pcs <br>(Kirim / Terima)</th>
...
<script type="text/javascript">
    $(function() {
        ...
        $('#supplier').select2({
            ...
            placeholder: "-- pilih --",
            ajax: {
                url: `finishing_cabut/get-supplier`,
                dataType: 'json',
                delay: 250,
        ...
        $.ajax({
            ...
            success: function(respon) {
                ...
                $(`#supplier`).select2("trigger", "select", {
                    data: {
                        id: respon.id_supplier,
                        text: respon.nama_supplier
                    }
                });
                $('#nomor').val(respon.nomor);
                $('#catatan').val(respon.catatan);
            },
    ...
    function tableParent() {
        table = $('#table-parent').DataTable({
            ...
            autoWidth: false,
            order: [],
            ajax: `finishing_cabut/table/${mode}`,
            ...
            columns: [{
                ...
                },
                {
                    ...
                    searchable: false,
                    render: (data) => {
                        let totalKirim = data.total_kirim || 0;
                        let totalTerima = data.total_terima || 0;
                        return `${totalKirim} / ${totalTerima}`;
                    }
                },
                {
                    data: null,
