...
<div class="form-group row">
    <div class="col-md-12">
        ...
        <button type="button" class="btn btn-primary btn-sm waves-effect waves-classic float-left mr-2"
            data-model-inspect="InspectFinishingCabutDetail" data-model-jasa-luar="FinishingCabutDetail"
            data-table-parent="finishing_cabut" data-route="inspect_finishing_cabut/get-barang"
            onclick="retur($(this))"><i class="icon md-plus mr-2"></i> Tambah (Retur)
        </button>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="padding-bottom: 20px;">
                <form class="form-horizontal" id="form" action="" method="POST"
                    ...
                    <input type="hidden" name="id_finishing_cabut" id="id_finishing_cabut">
...
<script type="text/javascript">
    $(function() {
        ...
        $('#spk').select2({
            ...
            ajax: {
                url: `inspect_finishing_cabut/get-spk`,
                dataType: 'json',
                delay: 250,
        ...
        selectGudang(`inspect_finishing_cabut/get-gudang`);
        $('#barang').select2({
            ...
            placeholder: "-- pilih --",
            ajax: {
                url: `inspect_finishing_cabut/get-barang`,
                data: function(d) {
                ...
                processResults: function(data) {
                    return {
                        results: data.data.map(function(data) {
                            return {
                                id: `${data.id_finishing_cabut},${data.id_mesin},${data.id_barang},${data.id_warna},${data.id_motif},${data.id_beam},${data.id_songket},${data.id_grade},${data.tanggal_potong}`,
                                ...
                                data: {
                                    id_finishing_cabut: data.id_finishing_cabut,
        ...
        $('#kualitas').select2({
            ...
            width: '100%',
            ajax: {
                url: `inspect_finishing_cabut/get-kualitas`,
                type: 'GET',
                dataType: 'json',
                delay: 250,
                data: function(d) {
    ...
    function cetak(this_) {
        var data = {
            ...
            proses: 'inspect_finishing_cabut',
        };
    ...
    function table() {
        table = $('#table').DataTable({
            ...
            order: [],
            ajax: {
                url: `inspect_finishing_cabut/table`,
                type: 'GET',
                data: function(d) {
                    d.spk = $('#spk').val();
    ...
    function getBarang(this_) {
        ...
        var id = $('#id').val();
        if (val) {
            var finishing_cabut = val.data.id_finishing_cabut;
            ...
            $('#id_finishing_cabut').val(finishing_cabut);
            $('#id_mesin').val(mesin);
    ...
    function edit(this_) {
        ...
        $.ajax({
            url: `inspect_finishing_cabut/get-data`,
            data: data,
            type: 'GET',
            success: function(data) {
                ...
                $(`#barang`).select2("trigger", "select", {
                    data: {
                        id: `${data.id_finishing_cabut},${data.id_mesin},${data.id_barang},${data.id_warna},${data.id_motif},${data.id_beam},${data.id_songket},${data.id_grade_awal},${data.tanggal_potong}`,
                        text: `${data.nomor} | ${data.id_mesin ? data.nama_mesin+' | ' : ''}${data.id_beam ? data.nomor_kikw + ' | ' : ''}${data.id_songket ? data.nomor_kiks + ' | ' : ''}${data.nama_barang} | ${data.nama_warna} | ${data.nama_motif} | ${data.nama_grade_awal}${data.tanggal_potong ? ' | '+data.tanggal_potong_text:''}`,
                        data: {
                            id_finishing_cabut: data.id_finishing_cabut,
                            id_mesin: data.id_mesin,
                            id_barang: data.id_barang,
                ...
                $('#id_grade_awal').val(data.id_grade_awal);
                editbarang =
                    `${data.id_finishing_cabut},${data.id_mesin},${data.id_barang},${data.id_warna},${data.id_motif},${data.id_beam},${data.id_songket},${data.id_grade},${data.id_grade_awal},${data.tanggal_potong}`;
                $(`#grade`).select2("trigger", "select", {
                    data: {
                        id: data.id_grade,
                        text: `${data.nama_grade} | ${data.alias_grade}`
                    }
                });
                $('#kualitas').empty();
                data.kualitas.forEach(function(data) {
    ...
    function getStok(data = {}, isRetur = false) {
        $.ajax({
            url: `inspect_finishing_cabut/get-stok-barang`,
            type: 'GET',
            data: data,
            success: function(data) {
                var id = data.id;
                if (id == "") {
                    if (isRetur) $('#stok_retur').text(data.stok_1);
                    $('#stok_1').text(data.stok_1);
                    $('#stok_2').text(data.stok_2);
                    $('#volume_1').val(data.stok_1);
                    $('#volume_2').val(data.stok_2);
                } else {
                    // var tempbarang = `${data.id_mesin},${data.id_barang},${data.id_warna},${data.id_motif},${data.id_beam},${data.id_songket},${data.id_grade},${data.id_grade_awal}`;
                    var tempbarang =
                    ...
                    if (editgudang != data.id_gudang || editbarang != tempbarang) {
                        ...
                        if (isRetur) $('#stok_retur').text(parseFloat(parseFloat(data.stok_1) + parseFloat(
                            $('#volume').val())));
                        $('#stok_1').text(parseFloat(parseFloat(data.stok_1) + parseFloat($('#volume_1')
                            .val())));
                        $('#stok_2').text(parseFloat(parseFloat(data.stok_2) + parseFloat($('#volume_2')
                            .val())));
                    }
                }
    ...
    /* function getStok(finishing_cabut, mesin, barang, warna, gudang, motif, beam, grade, id = null) {
        $.get(`inspect_finishing_cabut/get-stok-barang/${finishing_cabut}/${mesin}/${barang}/${warna}/${gudang}/${motif}/${beam}/${grade}`, function(data) {
            if (!id) {
                $('#stok_1').text(data.stok_1);
                $('#stok_2').text(data.stok_2);
                $('#volume_1').val(data.stok_1);
                ...
                var tempbarang = `${finishing_cabut},${mesin},${barang},${warna},${motif},${beam},${grade}`;
    ...
    function stok(finishing_cabut, mesin, barang, warna, gudang, motif, beam, grade) {
        var id = $('#id').val();
        if (id) {
            getStok(finishing_cabut, mesin, barang, warna, gudang, motif, beam, grade, id);
        } else {
            getStok(finishing_cabut, mesin, barang, warna, gudang, motif, beam, grade);
        }
    } */
</script>
