...
<table class="table table-bordered table-striped" cellspacing="0" id="table">
    <thead>
        <tr style="{{ $addStyle }} text-align:center;">
            <th style="{{ $addStyle }}" colspan="13">Persediaan Benang Warna per-Jenis ({{ tglIndo($tgl_awal) }} -
                {{ tglIndo($tgl_akhir) }})
            </th>
        </tr>
        <tr style="{{ $addStyle }} text-align:center;">
            ...
            <th style="{{ $addStyle }}" rowspan="3">WARNA</th>
    ...
    <tbody>
        ...
        @foreach ($data as $i)
            <tr style="{{ $addStyle }}">
                ...
                <td style="{{ $addStyle }} text-align:right;">{{ $i->nama_warna }}</td>
